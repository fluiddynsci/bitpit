...
#ifndef __BITPIT_VOLOCTREE_NON_UNIFORM_HPP__
#define __BITPIT_VOLOCTREE_NON_UNIFORM_HPP__
...
class VolOctreeNonUniform : public VolumeKernel {
public:
    ...
    struct OctantInfo {
        ...
        bool operator==(const OctantInfo& other) const { return (id == other.id && internal == other.internal); }
    ...
    struct OctantInfoHasher {
        ...
        // hash.
        std::size_t operator()(const OctantInfo& k) const { return std::hash<uint32_t>()(k.id); }
    };
    ...
    void simulateCellUpdate(long id, adaption::Marker marker, std::vector<Cell>* virtualCells,
                            PiercedVector<Vertex, long>* virtualVertices) const override;
...
protected:
    VolOctreeNonUniform(const VolOctreeNonUniform& other);
    ...
    void _findCellEdgeNeighs(long id, int edge, const std::vector<long>& blackList,
                             std::vector<long>* neighs) const override;
    void _findCellVertexNeighs(long id, int vertex, const std::vector<long>& blackList,
                               std::vector<long>* neighs) const override;
    ...
    std::vector<adaption::Info> _partitioningPrepare(const std::unordered_map<long, double>& cellWeights,
                                                     double defaultWeight, bool trackPartitioning) override;
...
private:
    ...
    struct RenumberInfo {
        RenumberInfo() : cellId(Cell::NULL_ID), newTreeId(0){};

        RenumberInfo(long _cellId, uint32_t _newTreeId) : cellId(_cellId), newTreeId(_newTreeId){};
    ...
    struct DeleteInfo {
        DeleteInfo() : cellId(Element::NULL_ID), trigger(adaption::TYPE_UNKNOWN), rank(-1){};
    ...
    struct FaceInfo {
        ...
        bool operator==(const FaceInfo& other) const { return (id == other.id && face == other.face); }
    ...
    struct FaceInfoHasher {
        ...
        // in the hash.
        std::size_t operator()(const FaceInfo& k) const { return std::hash<long>()(k.id); }
    };
    ...
    std::vector<long> importCells(const std::vector<OctantInfo>& octantTreeIds, StitchInfo& stitchInfo,
...
#if BITPIT_ENABLE_MPI == 1
    void initializeTree(std::unique_ptr<PabloNonUniform>* adopter, std::size_t haloSize);
    void initializeTreePartitioning();
    void initializeTreeHaloSize(std::size_t haloSize);
#else
    void initializeTree(std::unique_ptr<PabloNonUniform>* adopter);
#endif
};
