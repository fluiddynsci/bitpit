...
#endif
{
    ...
    m_tree = std::unique_ptr<PabloUniform>(
        new PabloUniform(origin[0], origin[1], origin[2], length, dimension, PabloUniform::DEFAULT_LOG_FILE));
#endif
...
*/
VolOctree::VolOctree(const VolOctree& other) : VolumeKernel(other), virtualOctantsDummy(other.virtualOctantsDummy) {
...
*/
void VolOctree::simulateCellUpdate(long id, adaption::Marker marker, std::vector<Cell>* virtualCells,
...
/*!
        ...
        found by this function
*/
void VolOctree::_findCellEdgeNeighs(long id, int edge, const std::vector<long>& blackList,
...
/*!
        ...
        found by this function
*/
void VolOctree::_findCellVertexNeighs(long id, int vertex, const std::vector<long>& blackList,
                                      std::vector<long>* neighs) const {
    // Get octant info
    const OctantInfo octantInfo = getCellOctant(id);
