...
#include "voloctree_non_uniform.hpp"

namespace bitpit {

/*!
	\class VolOctreeNonUniform
	\ingroup volumepatches

	\brief The VolOctreeNonUniform defines a Octree patch.

	VolOctreeNonUniform defines a Octree patch.
*/

/*!
	\struct VolOctreeNonUniform::OctantInfo
	\ingroup volumepatches
...
/*!
	\struct VolOctreeNonUniform::OctantInfoHasher
	\ingroup volumepatches
...
*/
VolOctreeNonUniform::VolOctreeNonUniform()
	: VolumeKernel(false)
#endif
{
	// Create the tree
...
/*!
	...
	cells halo
*/
VolOctreeNonUniform::VolOctreeNonUniform(int dimension, const std::array<double, 3> &origin, darray3 mL, double dh, MPI_Comm communicator, std::size_t haloSize)
...
VolOctreeNonUniform::VolOctreeNonUniform(int dimension, const std::array<double, 3> &origin, darray3 mL, double dh)
	: VolOctreeNonUniform(PatchManager::AUTOMATIC_ID, dimension, origin, mL, dh)
#endif
{
}
...
/*!
	...
	cells halo
*/
VolOctreeNonUniform::VolOctreeNonUniform(int id, int dimension, const std::array<double, 3> &origin, darray3 mL, double dh, MPI_Comm communicator, std::size_t haloSize)
...
/*!
	...
	\param dh is the maximum allowed cell size of the initial refinement
*/
VolOctreeNonUniform::VolOctreeNonUniform(int id, int dimension, const std::array<double, 3> &origin, darray3 mL, double dh)
...
#endif
{
	...
	m_tree = std::unique_ptr<PabloNonUniform>(
	    new PabloNonUniform(origin[0], origin[1], origin[2], mL, dimension,
	                     PabloNonUniform::DEFAULT_LOG_FILE, communicator));
#else
	m_tree = std::unique_ptr<PabloNonUniform>(
	    new PabloNonUniform(origin[0], origin[1], origin[2], mL, dimension,
	...
	if (m_tree->getNumOctants() > 0) {
        double length = std::max(mL[0], std::max(mL[1], mL[2]));
...
/*!
	...
	\param stream is the stream to read from
*/
VolOctreeNonUniform::VolOctreeNonUniform(std::istream &stream)
	: VolumeKernel(false)
#endif
{
	// Initialize the tree
...
/*!
	...
	\param adopter is a pointer to the tree adopter
*/
VolOctreeNonUniform::VolOctreeNonUniform(int id, std::unique_ptr<PabloNonUniform> &&tree, std::unique_ptr<PabloNonUniform> *adopter)
#if BITPIT_ENABLE_MPI==1
...
*/
VolOctreeNonUniform::VolOctreeNonUniform(const VolOctreeNonUniform &other)
	: VolumeKernel(other)
{
...
*/
std::unique_ptr<PatchKernel> VolOctreeNonUniform::clone() const
{
	return std::unique_ptr<VolOctreeNonUniform>(new VolOctreeNonUniform(*this));
}

/*!
	Internal function to reset the patch.
*/
void VolOctreeNonUniform::reset()
{
...
/*!
	Reset the patch.
*/
void VolOctreeNonUniform::__reset(bool resetTree)
{
	// Reset the tree
	if (resetTree) {
		m_tree->reset();
...
/*!
	...
	cells halo
*/
void VolOctreeNonUniform::initializeTree(std::unique_ptr<PabloNonUniform> *adopter, std::size_t haloSize)
#else
void VolOctreeNonUniform::initializeTree(std::unique_ptr<PabloNonUniform> *adopter)
#endif
{
#if BITPIT_ENABLE_MPI==1
...
*/
void VolOctreeNonUniform::initialize()
{
...
*/
void VolOctreeNonUniform::setDimension(int dimension)
{
...
/*!
	...
	\param dimension the dimension of the patch
*/
void VolOctreeNonUniform::__setDimension(int dimension)
{
...
*/
void VolOctreeNonUniform::simulateCellUpdate(long id, adaption::Marker marker, std::vector<Cell> *virtualCells, PiercedVector<Vertex, long> *virtualVertices) const
{
...
*/
double VolOctreeNonUniform::evalCellVolume(long id) const
{
	OctantInfo octantInfo = getCellOctant(id);
	const Octant *octant = getOctantPointer(octantInfo);
...
*/
std::array<double, 3> VolOctreeNonUniform::evalCellCentroid(long id) const
{
	OctantInfo octantInfo = getCellOctant(id);
	const Octant *octant = getOctantPointer(octantInfo);
...
*/
void VolOctreeNonUniform::evalCellBoundingBox(long id, std::array<double,3> *minPoint, std::array<double,3> *maxPoint) const
{
	OctantInfo octantInfo = getCellOctant(id);
	const Octant *octant = getOctantPointer(octantInfo);
...
*/
double VolOctreeNonUniform::evalCellSize(long id) const
{
	OctantInfo octantInfo = getCellOctant(id);
	const Octant *octant = getOctantPointer(octantInfo);
...
*/
double VolOctreeNonUniform::evalInterfaceArea(long id) const
{
...
*/
std::array<double, 3> VolOctreeNonUniform::evalInterfaceNormal(long id) const
{
	const Interface &interface = getInterface(id);
...
*/
VolOctreeNonUniform::OctantInfo VolOctreeNonUniform::getCellOctant(long id) const
{
...
*/
PabloNonUniform & VolOctreeNonUniform::getTree()
{
	return *m_tree;
}
...
*/
const PabloNonUniform & VolOctreeNonUniform::getTree() const
{
	return *m_tree;
}
...
/*!
	...
	\param adopter is a pointer to the tree adopter
*/
void VolOctreeNonUniform::setTreeAdopter(std::unique_ptr<PabloNonUniform> *adopter)
{
...
*/
long VolOctreeNonUniform::getOctantId(const OctantInfo &octantInfo) const
{
...
*/
Octant * VolOctreeNonUniform::getOctantPointer(const OctantInfo &octantInfo)
{
	Octant *octant;
	if (octantInfo.internal) {
...
*/
const Octant * VolOctreeNonUniform::getOctantPointer(const OctantInfo &octantInfo) const
{
    const Octant *octant;
    if (octantInfo.internal) {
        octant = m_tree->getOctant(octantInfo.id);
...
*/
VolOctreeNonUniform::OctantHash VolOctreeNonUniform::evaluateOctantHash(const OctantInfo &octantInfo)
{
...
*/
int VolOctreeNonUniform::getCellLevel(long id) const
{
	OctantInfo octantInfo = getCellOctant(id);
...
*/
int VolOctreeNonUniform::getCellFamilySplitLocalVertex(long id) const
{
	OctantInfo octantInfo = getCellOctant(id);
	const Octant *octant = getOctantPointer(octantInfo);
...
*/
std::vector<adaption::Info> VolOctreeNonUniform::_spawn(bool trackSpawn)
{
...
*/
std::vector<adaption::Info> VolOctreeNonUniform::_adaptionPrepare(bool trackAdaption)
{
...
*/
std::vector<adaption::Info> VolOctreeNonUniform::_adaptionAlter(bool trackAdaption)
{
	// Updating the tree
...
*/
void VolOctreeNonUniform::_adaptionCleanup()
{
	// Nothing to do
}
...
*/
void VolOctreeNonUniform::settleAdaptionMarkers()
{
...
*/
std::vector<adaption::Info> VolOctreeNonUniform::sync(bool trackChanges)
{
	...
	while (treeId < (uint32_t) nOctants) {
		...
		// only happens inside VolOctreeNonUniform.
...
*/
VolOctreeNonUniform::StitchInfo VolOctreeNonUniform::deleteCells(const std::vector<DeleteInfo> &deletedOctants)
{
	// Info of the cells
	int nCellVertices = m_cellTypeInfo->nVertices;
...
/*!
	...
	octants
*/
void VolOctreeNonUniform::renumberCells(const std::vector<RenumberInfo> &renumberedOctants)
{
...
*/
std::vector<long> VolOctreeNonUniform::importCells(const std::vector<OctantInfo> &octantInfoList,
...
*/
void VolOctreeNonUniform::_updateAdjacencies()
{
	// Face information
...
*/
bool VolOctreeNonUniform::_markCellForRefinement(long id)
{
...
*/
bool VolOctreeNonUniform::_markCellForCoarsening(long id)
{
	return setMarker(id, -1);
}
...
*/
bool VolOctreeNonUniform::_resetCellAdaptionMarker(long id)
{
...
*/
adaption::Marker VolOctreeNonUniform::_getCellAdaptionMarker(long id)
{
	OctantInfo octantInfo = getCellOctant(id);
	if (!octantInfo.internal) {
...
*/
bool VolOctreeNonUniform::setMarker(long id, int8_t value)
{
	OctantInfo octantInfo = getCellOctant(id);
	if (!octantInfo.internal) {
		return false;
...
*/
bool VolOctreeNonUniform::_enableCellBalancing(long id, bool enabled)
{
	OctantInfo octantInfo = getCellOctant(id);
	if (!octantInfo.internal) {
		return false;
 ...
 */
bool VolOctreeNonUniform::isPointInside(const std::array<double, 3> &point) const
{
	bool isGhost;
 ...
 */
bool VolOctreeNonUniform::isPointInside(long id, const std::array<double, 3> &point) const
{
...
*/
long VolOctreeNonUniform::locatePoint(const std::array<double, 3> &point) const
{
	bool isGhost;
...
/*!
	...
	checks
*/
void VolOctreeNonUniform::_setTol(double tolerance)
{
...
*/
void VolOctreeNonUniform::_resetTol()
{
	m_tree->setTol();
 ...
 */
int VolOctreeNonUniform::_getDumpVersion() const
{
 ...
 */
void VolOctreeNonUniform::_dump(std::ostream &stream) const
{
	// List all octants
 ...
 *  \param stream is the stream to read from
 */
void VolOctreeNonUniform::_restore(std::istream &stream)
{
	// Restore tree
#if ENABLE_MPI==1
...
*/
long VolOctreeNonUniform::_getCellNativeIndex(long id) const
{
	OctantInfo octantInfo = getCellOctant(id);
...
*/
std::array<double, 3> VolOctreeNonUniform::getOrigin() const
{
...
*/
void VolOctreeNonUniform::setOrigin(const std::array<double, 3> &origin)
{
 ...
 */
void VolOctreeNonUniform::translate(const std::array<double, 3> &translation)
{
...
*/
darray3 VolOctreeNonUniform::getLength() const
{
	return m_tree->getL();
}
...
*/
void VolOctreeNonUniform::setLength(darray3 length)
{
	// Set the length
	m_tree->setL(length);
 ...
 */
void VolOctreeNonUniform::scale(const std::array<double, 3> &scaling, const std::array<double, 3> &center)
{
	...
	if (!uniformScaling) {
		log::cout() << "VolOctreeNonUniform patch only allows uniform scaling." << std::endl;
		return;
	}
...
*/
void VolOctreeNonUniform::_findCellNeighs(long id, const std::vector<long> *blackList, std::vector<long> *neighs) const
{
	OctantInfo octantInfo = getCellOctant(id);
...
/*!
	...
	found by this function
*/
void VolOctreeNonUniform::_findCellEdgeNeighs(long id, int edge, const std::vector<long> *blackList, std::vector<long> *neighs) const
{
...
/*!
	...
	found by this function
*/
void VolOctreeNonUniform::_findCellVertexNeighs(long id, int vertex, const std::vector<long> *blackList, std::vector<long> *neighs) const
{
	// Get octant info
	const OctantInfo octantInfo = getCellOctant(id);
...
*/
void VolOctreeNonUniform::findOctantCodimensionNeighs(const OctantInfo &octantInfo, int index, int codimension,
