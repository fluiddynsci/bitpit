...
#endif
{
    ...
    m_tree = std::unique_ptr<PabloNonUniform>(
        new PabloNonUniform(origin[0], origin[1], origin[2], mL, dimension, PabloUniform::DEFAULT_LOG_FILE));
#endif
...
*/
VolOctreeNonUniform::VolOctreeNonUniform(const VolOctreeNonUniform& other) : VolumeKernel(other), virtualOctantsDummy(other.virtualOctantsDummy) {
...
*/
void VolOctreeNonUniform::simulateCellUpdate(long id, adaption::Marker marker, std::vector<Cell>* virtualCells,
    ...
    octvector virtualOctants = virtualOctantsDummy;
...
std::vector<adaption::Info> VolOctreeNonUniform::_adaptionPrepare(bool trackAdaption) {
    ...
    if(trackAdaption) {
        ...
        if(isPartitioned() && getGhostCellCount() > 0) {
            std::size_t adaptionInfoId =
                adaptionData.create(adaption::TYPE_DELETION, adaption::ENTITY_CELL, currentRank);
...
std::vector<adaption::Info> VolOctreeNonUniform::sync(bool trackChanges) {
...
#if BITPIT_ENABLE_MPI == 1
        } else if(lastTreeOperation == ParaTree::OP_LOADBALANCE ||
                  lastTreeOperation == ParaTree::OP_LOADBALANCE_FIRST) {
    ...
    if(deletedOctants.size() > 0) {
        ...
        if(trackChanges) {
            ...
            std::size_t adaptionInfoId =
                adaptionData.create(adaption::TYPE_DELETION, adaption::ENTITY_INTERFACE, currentRank);
            adaption::Info& adaptionInfo = adaptionData[adaptionInfoId];
    ...
    if(trackChanges) {
        ...
        if(nGhostsOctants > 0) {
            std::size_t adaptionInfoId =
                adaptionData.create(adaption::TYPE_CREATION, adaption::ENTITY_CELL, currentRank);
            adaption::Info& adaptionInfo = adaptionData[adaptionInfoId];
...
*/
std::vector<long> VolOctreeNonUniform::importCells(const std::vector<OctantInfo>& octantInfoList, StitchInfo& stitchInfo,
...
/*!
        ...
        found by this function
*/
void VolOctreeNonUniform::_findCellEdgeNeighs(long id, int edge, const std::vector<long>& blackList,
...
/*!
        ...
        found by this function
*/
void VolOctreeNonUniform::_findCellVertexNeighs(long id, int vertex, const std::vector<long>& blackList,
                                      std::vector<long>* neighs) const {
    // Get octant info
    const OctantInfo octantInfo = getCellOctant(id);
